...
async function execJob(cm, ...args) {
  ...
  return [rt, conn.bytesSent, conn.bytesReceived];
}
...
class ARsshClient extends EventEmitter {
  ...
  async exec(cmd, opt = {}, stdout = null, stderr = null) {
    ...
    return this._batch.qsubAndWait({
      exec: async()=>{
        this._prologue();
        const [rt, sent, received] = await execJob(this._cm, cmd, opt, stdout, stderr);
        this.bytesSent = this.bytesSent + sent;
        this.bytesReceived = this.bytesReceived + received;
        this._epilogue();
        return rt;
      },
      name: "exec"
  ...
  _epilogue() {
    debug("_epilogue() is not impimented");
  }
}
