...
class Executer extends EventEmitter {
  ...
  async _exec(ssh, order) {
    return ssh.exec(order.cmd, order.opt, order.stdout, order.stderr);
  }
  ...
  async _get(ssh, order) {
    ...
    if (!srcIsFile && !srcIsDir) {
      return Promise.reject(
        new Error("src must be existing file or directory")
      );
    } else if (srcIsDir) {
    ...
    await this._getCmd(order.type)(conn.ssh, order)
      ...
      .catch(err => {
        ...
        if (
          ...
          err.message ===
            "You should wait continue event before sending any more traffic"
        ) {
...
class ARsshClient {
  ...
  off(eventName, listener) {
    return this.cm.off(eventName, listener);
  }
