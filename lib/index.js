...
class ARsshClient extends EventEmitter {
   ...
   * @param {number} duration - duration between each try (in msec)
   ...
   * @param {[]|Function} stderr
   */
  async watch(cmd, regexp, duration = 30000000, maxRetry = null, opt = {}, stdout = null, stderr = null) {
    if (opt === null) {
      opt = {};
    }
    debug("wait called", cmd, regexp, duration, maxRetry, opt);

    if (typeof cmd !== "string") {
      return Promise.reject(new Error("cmd must be string"));
    }
    const regexpOut = regexp.hasOwnProperty("out") && regexp.out instanceof RegExp ? regexp.out : regexp instanceof RegExp ? regexp : null;
    const regexpErr = regexp.hasOwnProperty("err") && regexp.err instanceof RegExp ? regexp.err : regexp instanceof RegExp ? regexp : null;
    ...
    if (regexpOut === null && regexpErr === null) {
      ...
      err.cmd = cmd;
      err.duration = err.duration;
      err.maxRetry = err.maxRetry;
      err.stdout = stdout;
      err.stderr = stderr;
