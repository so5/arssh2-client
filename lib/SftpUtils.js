...
class SftpUtil{
  constructor(sftp){
    ...
    this.functions = {
      ...
      readdir   : { put: promisify(fs.readdir),     get: this.readdirAdoptor.bind(this) },
      stat      : { put: promisify(fs.stat),        get: this.stat.bind(this) },
      optSetter : { put: this.optSetter.bind(this), get: this.dummyOptSetter.bind(this)}
    }
  }
  async dummyOptSetter(opt, stat, file){
    return opt;
  }
  ...
  async _putget(mode, src, dst, opt={}){
    ...
    let optSetter = this.functions.optSetter[mode];
    ...
    if(dstIsDir){
      dst = path.join(dst, path.posix.basename(src));
    }

    let optModified = await optSetter(opt, stat, src);
    return func(src, dst, optModified)
      .catch((e)=>{
        e.func=func
        e.src=src;
        e.dst=dst;
