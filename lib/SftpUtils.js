...
const { mkdirIfNotExist, mkdir_pLocal, walk } = require("./utils");

function replacePathsep(oldPath) {
  if (path.sep === path.win32.sep) {
    let newPath = oldPath.replace(new RegExp("\\" + path.win32.sep, "g"), path.posix.sep);
    return newPath;
  } else {
    return oldPath;
  }
...
class SftpUtil {
   ...
   */
  async isDir(target) {
    return this._checkStat(target, S_ISDIR);
  }
   ...
   */
  async isFile(target) {
    return this._checkStat(target, S_ISREG);
  }
  ...
  async getSize(target) {
    return this._checkStat(target, returnSize);
  }

  /**
   * get absolute path on remote serever
   * @param {string} target - directory or filename which you want to convert
   */
  async realpath(target) {
    if (target.endsWith("/")) target = target.slice(0, -1);
    if (target.endsWith("\\")) target = target.slice(0, -1);
    return new Promise((resolve, reject) => {
      this.sftp.realpath(target, (err, absPath) => {
        if (err) {
          reject(err);
        }
        resolve(absPath);
  ...
  async _checkStat(target, parser) {
    return new Promise((resolve, reject) => {
      this.stat(target, (err, stat) => {
        if (err) {
  ...
  async put_R(src, dst, only, exclude, opt) {
    ...
    if (only || exclude) debug("only filter=", only, " exclude filter =", exclude);
  ...
  async _putget(
    ...
    if (!await isFileSrc(src)) return Promise.reject(new Error("src must be file"));
    let dstFile = dst;

    if (dst.endsWith("/") || dst.endsWith("\\") || (await isDirDst(dst))) {
      dstFile = pathDst.join(dst, pathSrc.basename(src));
    }
    let opt2 = opt;
    if (optSetter) {
      opt2 = await optSetter(opt, stat, src);
    }
    if (only !== null && !minimatch(src, only)) {
      debug("_putget:", src, "canceled because it does not match only filter");
      return Promise.resolve();
    }
    if (exclude !== null && minimatch(src, exclude)) {
      debug("_putget:", src, "canceled because it match exclude filter");
      return Promise.resolve();
    }
    debug("_putget: from", src, "to", dstFile);
    await mkdirIfNotExist(isDirDst, mkdir_p, pathDst.dirname(dstFile));
    return func(src, dstFile, opt2).catch((e) => {
      e.func = func;
      e.src = src;
      e.dst = dst;
      e.opt = opt;
  ...
  async _putget_R(
    ...
    await srcDirs.reduce((p, e) => {
      return p.then(mkdirIfNotExist.bind(this, isDirDst, mkdir, pathDst.join(dst, pathSrc.relative(src, e))));
    }, Promise.resolve());
    ...
    let pFile = srcFiles.map((srcFile, i) => {
      let dstFile = pathDst.join(dst, replacePathsep(pathSrc.relative(src, srcFile)));
      if (only !== null && !minimatch(srcFile, only)) {
        debug("_putget_R:", srcFile, "canceled because  it does not match only filter");
        return Promise.resolve();
      }
      if (exclude !== null && minimatch(srcFile, exclude)) {
        debug("_putget_R:", srcFile, "canceled because it match exclude filter");
        return Promise.resolve();
      }
