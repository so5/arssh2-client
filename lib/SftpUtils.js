...
async function readdir(sftp, target) {
  const files = await readdir_org(sftp, target);
  return files.map((e)=>{
    return e.filename;
  });
}

async function readdir_org(sftp, ...orgArgs) {
  return promisify2(sftp.readdir.bind(sftp), ...orgArgs);
}
