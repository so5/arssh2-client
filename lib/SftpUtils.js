...
class SftpUtil{
   ...
   */
  async ls(target){
    let target2=target.replace(path.win32.sep, path.posix.sep);
    if(await this.isDir(target2)){
      debug('ls dir', target2);
      let attrs = await this.readdir(target2);
      let rt = attrs.map((e)=>{
        return e.filename
      });
      return rt;
    }else{
      debug('ls file', target2);
      try{
        await this.stat(target2)
      }catch(err){
      ...
      return [path.posix.basename(target2)]
    }
  }
  ...
  async put_R(src, dst, opt){
    return this._putget_R(this.fastPut.bind(this),
  ...
  async get_R(src, dst, opt){
    return this._putget_R(this.fastGet.bind(this),
      isDirLocal,
      isFileLocal,
      mkdir_pLocal,
      promisify(fs.mkdir),
  ...
  async put(src, dst, opt){
    return  this._putget(this.fastPut.bind(this), 
      this.isDir.bind(this),
      isFileLocal,
      this.mkdir_p.bind(this),
      promisify(fs.stat),
  ...
  async get(src, dst, opt){
    return  this._putget(this.fastGet.bind(this),
      isDirLocal,
      this.isFile.bind(this), 
      mkdir_pLocal,
      this.stat.bind(this),
   ...
   */
  async _putget(func, isDirDst, isFileSrc, mkdir_p, stat, pathDst, pathSrc, optSetter, src, dst, opt={}){
    // velify src and dst
   ...
   * @param { Object } opt - option object of func
   */
  async _putget_R(func, isDirDst, isFileDst, mkdir_p, mkdir, readdir, stat, pathDst, pathSrc, optSetter, src, dst, opt={}){
    ...
    let pFile = srcFiles.map((srcFile, i)=>{
      let dstFile = pathDst.join(dst, pathSrc.relative(src, srcFile).replace(path.win32.sep, path.posix.sep));
      return func(srcFile, dstFile, opts[i])
        .catch((e)=>{
          e.func=func
          e.src=src;
          e.dst=dst;
