...
async function _promisify(fn, ...orgArgs) {
  return new Promise((resolve, reject) => {
    ...
    if (!rt) reject(new Error("wait for continue"));
  });
}
...
async function put(sftp, src, dst, only, exclude, opt = {}) {
  ...
  try {
    ...
    if (e.code !== "EISDIR") return Promise.reject();
  }
...
async function rput(sftp, src, dst, only, exclude, opt = {}) {
  ...
  // transfer files
  let pFile = srcFiles.map(async (srcFile) => {
    let dstFile = path.posix.join(dst, _replacePathsep(path.relative(src, srcFile)));
...
async function get(sftp, src, dst, only, exclude, opt = {}) {
  debug("get", src, "to", dst, "with options\n", JSON.stringify({ only: only, exclude: exclude, opt: opt }, null, 2));
  // velify src
