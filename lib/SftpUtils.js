...
class SftpUtil {
  ...
  async put_R(src, dst, only, exclude, opt) {
    ...
    if (only || exclude)
      debug("only filter=", only, " exclude filter =", exclude);
  ...
  async _putget(
    ...
    if (!await isFileSrc(src))
      return Promise.reject(new Error("src must be file"));
    let dstFile = dst;
    ...
    let pFile = srcFiles.map((srcFile, i) => {
      ...
      if (only !== null && !minimatch(srcFile, only)) {
        debug(
          "_putget_R:",
          srcFile,
          "canceled because  it does not match only filter"
        );
        return Promise.resolve();
      }
      if (exclude !== null && minimatch(srcFile, exclude)) {
        debug(
          "_putget_R:",
          srcFile,
          "canceled because it match exclude filter"
        );
        return Promise.resolve();
      }
